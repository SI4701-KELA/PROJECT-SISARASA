...
class PortionSelectionTest extends DuskTestCase
{
    /**
    ...
    /**
     * TC-POR-002: Menguji validasi pengurangan porsi di bawah batas minimal (Negative)
     */
    public function test_decrement_portion_validation(): void
    {
        $user = $this->getOrCreateBuyer();
        $product = $this->getOrCreateProduct();
        ...
        $this->browse(function (Browser $browser) use ($user, $product): void {
            $browser->loginAs($user)
                ...
                ->waitForText($product->name, 10)
                ->assertSeeIn('.flex-1.bg-white span', '0') // Porsi awal = 0
                ->click('.flex-1.bg-white button:nth-child(1)') // Klik [-]
                ->assertSeeIn('.flex-1.bg-white span', '0'); // Tetap 0
        });
    }

    /**
     * TC-POR-003: Menguji validasi input porsi jika melebihi sisa stok (Negative)
     */
    public function test_maximum_stock_validation(): void
    {
        $user = $this->getOrCreateBuyer();
        $product = $this->getOrCreateProduct();
        $maxStock = $product->stock->qty_reg;

        $this->browse(function (Browser $browser) use ($user, $product, $maxStock): void {
            $browser->loginAs($user)
                ->visit('/buyer/menu')
                ->waitForText($product->name, 10)
                ->assertSeeIn('.flex-1.bg-white span', '0');

            // Klik tombol [+] sebanyak stok + 1 kali
            for ($i = 0; $i < $maxStock + 1; $i++) {
                $browser->click('.flex-1.bg-white button:nth-child(3)');
            }

            // Nilai porsi otomatis terkunci pada angka stok maksimalnya
            $browser->assertSeeIn('.flex-1.bg-white span', (string)$maxStock);
        });
    }
